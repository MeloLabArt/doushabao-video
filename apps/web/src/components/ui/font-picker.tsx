...
export function FontPicker({
	...
	const tabLabelMap: Record<string, string> = {
		all: t("fontPicker.allFonts"),
		"my-fonts": t("fontPicker.myFonts"),
		favorites: t("fontPicker.favorites"),
	};
	const activeTabLabel = tabLabelMap[activeTab]?.toLowerCase() ?? "";
		...
		<Popover open={open} onOpenChange={setOpen}>
			...
			>
				<div className="flex min-w-0 items-center gap-1.5">
					...
					<span className="truncate" style={{ fontFamily: defaultValue }}>
						{defaultValue ?? t("fontPicker.selectFont")}
					</span>
				</div>
			...
			>
				<div className="relative px-3 py-1.5">
					...
					<Input
						ref={searchInputRef}
						placeholder={t("fontPicker.searchPlaceholder", { tab: activeTabLabel })}
						value={search}
				...
				{status === "loading" && (
					<div className="py-8 text-center text-sm text-muted-foreground">
						{t("fontPicker.loadingFonts")}
					</div>
				)}
				{status === "error" && (
					<div className="flex flex-col items-center gap-3 py-8 px-4">
						<p className="text-sm text-muted-foreground text-center">
							{t("ui.failedLoadFonts")}
						</p>
